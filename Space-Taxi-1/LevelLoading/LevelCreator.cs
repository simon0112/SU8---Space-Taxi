...
namespace SpaceTaxi_1.LevelLoading {
    public class LevelCreator {
        ...
        public Level CreateLevel(string levelname) {
            ...
            for (int y = reader.MapData.Count-1; y >= 0; y--) {
                for (int x = 0; x < reader.MapData[y].Length; x++) {
                    var ImageIndex = reader.LegendData.IndexOf(reader.MapData[y].Substring(x,x+1))+1;
                    var ImageName = reader.LegendData[ImageIndex].Substring(3);
                    level.AddObstacle((new DIKUArcade.Entities.StationaryShape((x/reader.MapData[y].Length),(y/reader.MapData.Count),(1/reader.MapData[y].Length),(1/reader.MapData.Count))), new DIKUArcade.Graphics.Image(Path.Combine("Assets", "Images", ImageName)));
                }
            }
